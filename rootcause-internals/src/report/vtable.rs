...
impl ReportVtable {
    ...
    pub(super) unsafe fn source<'a>(
        ...
        // SAFETY: We know that the `self.source` field points to the function `source::<C>` below.
        // The safety requirement of that function is that the `C` matches the one stored in the
        // `ReportData` pointed to by `ptr`. This is guaranteed by the caller of this method.
...
/// - The caller must ensure that the pointer comes from an [`triomphe::Arc<ReportData<C>>`], which was turned
///   into a pointer using [`triomphe::Arc::into_raw`].
/// - The context type `C` must match the actual context type stored in the [`ReportData`].
/// - After calling this method, the pointer must no longer be used.
pub(super) unsafe fn drop<C: 'static>(ptr: NonNull<ReportData<Erased>>) {
    ...
    // - The given pointer must be of the correct type and have come from a call to `Arc::into_raw`.
    ...
    // references to the same `ReportData` instance, as this method consumes the pointer.
...
unsafe fn clone_arc<C: 'static>(ptr: NonNull<ReportData<Erased>>) -> RawReport {
    ...
    // SAFETY: Our caller guarantees that we point to a `ReportData<C>` and that this pointer came
    // from a `triomphe::Arc::into_raw`.
    //
    // This fulfills the safety docs for `ArcBorrow::from_ptr`, which explicitly mentions
    // the `as_ptr` (which is called from `into_raw`) is safe.
...
unsafe fn source<'a, C: 'static, H: ContextHandler<C>>(
    ...
    // SAFETY: Our caller guarantees that the type `C` matches the actual context type stored in the `ReportData`.
