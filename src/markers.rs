...
//!
//! This module provides type-level markers that control how reports and attachments
//! behave with respect to ownership, cloning, and thread safety. These markers are
//! used as generic parameters in types like [`Report<C, O, T>`](crate::Report) to
//! enforce compile-time guarantees about how data can be accessed and shared.
//!
//! # Ownership Markers
//!
//! Ownership markers control whether reports and report references can be mutated and cloned.
//!
//! For owned reports ([`Report<C, O, T>`](crate::Report)), the ownership marker `O` can be:
...
//!
//! For report references ([`ReportRef<C, O>`](crate::ReportRef)), the ownership marker `O` can be:
//! - [`Cloneable`]: Enables [`clone_arc`](crate::ReportRef::clone_arc) to get an owned report
//! - [`Uncloneable`]: Does not provide [`clone_arc`](crate::ReportRef::clone_arc)
//!
//! # Thread Safety Markers
//!
//! Thread safety markers control whether reports can be sent between threads or shared
//! across threads. These appear as the third type parameter (`T`) in [`Report<C, O, T>`](crate::Report):
//!
//! - [`SendSync`]: The report and all its contents are `Send + Sync`, allowing the
//!   report to cross thread boundaries.
//! - [`Local`]: The report contains non-thread-safe data (like `Rc` or raw pointers)
//!   and cannot be sent between threads.
//!
//! # Examples
//!
...
//! let local_report: Report<Rc<String>, markers::Mutable, markers::Local> =
//!     report!(local_data);
...
///
/// This marker is used exclusively with [`Report<C, Mutable, T>`](crate::Report) (not
/// [`ReportRef`](crate::ReportRef)). It indicates that the report has unique ownership of
/// its data, which allows mutation operations but prevents cloning.
///
...
/// - Convert to [`Cloneable`] with [`into_cloneable`](crate::Report::into_cloneable)
///
/// # Examples
///
/// ```
...
///     report!("Database error".to_string());
///
/// // Can add attachments (consumes and returns the report)
/// let report: Report<String, markers::Mutable> =
///     report.attach("connection timeout");
///
...
/// [`ReportRef<C, Cloneable>`](crate::ReportRef). It indicates shared ownership using
/// reference counting (`Arc` internally), which allows cheap cloning but prevents mutation.
///
/// # Usage with Report
///
/// For [`Report<C, Cloneable, T>`](crate::Report), this marker means the report itself
/// implements `Clone`, allowing you to cheaply clone the entire report (shallow copy via `Arc`).
///
/// # Usage with ReportRef
///
/// For [`ReportRef<C, Cloneable>`](crate::ReportRef), the marker enables the
/// [`clone_arc`](crate::ReportRef::clone_arc) method, which clones the underlying `Arc`
/// to produce an owned [`Report<C, Cloneable, T>`](crate::Report). Note that `ReportRef`
/// itself is always `Copy` and `Clone` regardless of the ownership marker - the `Cloneable`
/// marker specifically enables converting the reference back to an owned report.
///
/// # When to Use
///
...
///     report!("Error".to_string()).into_cloneable();
///
...
///
/// This marker is used exclusively with [`ReportRef<C, Uncloneable>`](crate::ReportRef)
/// (not [`Report`](crate::Report)). It indicates that the reference does not provide the
/// [`clone_arc`](crate::ReportRef::clone_arc) method to obtain an owned report.
///
/// Note that `ReportRef` itself is always `Copy` and `Clone` - you can always copy the
/// reference itself. The `Uncloneable` marker only prevents cloning the underlying `Arc`
...
///    `ReportRef<C, Uncloneable>` because the underlying report has unique ownership.
///
/// 2. **Explicitly restricting cloneability**: You can convert a `ReportRef<C, Cloneable>`
///    to `ReportRef<C, Uncloneable>` when you want to pass a reference that explicitly
///    cannot use `clone_arc`, ensuring the recipient can only inspect the report without
///    obtaining ownership. This can be useful in APIs that need to accept both cloneable
///    and uncloneable references.
///
///
/// # Examples
///
...
/// let uncloneable_ref: ReportRef<String, markers::Uncloneable> =
///     cloneable_ref.into_uncloneable();
/// ```
#[derive(Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Debug, Default, Hash)]
...
///
/// This is the default thread-safety marker for reports. When all the context objects
/// and attachments in a report implement `Send + Sync`, the report itself can be safely
/// sent to other threads and shared between threads.
///
/// # When to Use
///
...
///
/// Use `SendSync` (the default) unless you have a specific need for thread-local data.
///
/// # Examples
///
/// ```
...
///
/// This marker is used when a report contains thread-local data that cannot be safely
/// sent between threads or shared across threads. Common examples include `Rc`, raw
/// pointers, or types that explicitly opt out of `Send`/`Sync`.
///
/// # When to Use
///
...
/// let report: Report<Rc<String>, markers::Mutable, markers::Local> =
///     report!(local_data);
///
...
/// let local_report: Report<String, markers::Mutable, markers::Local> =
///     report.into_local();
/// ```
#[derive(Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Debug, Default, Hash)]
pub struct Local;
...
///
/// This trait is automatically implemented for all `Sized + 'static` types, and also for
/// the special type `dyn Any` which is used to represent type-erased contexts and attachments.
///
/// # Type Erasure with `dyn Any`
///
/// The `dyn Any` marker is special: it indicates a type-erased value without actually
/// creating a `Box<dyn Any>` or similar allocation. Converting from a concrete type to
/// `dyn Any` is a zero-cost operation that doesn't change the underlying representation.
///
/// This is used internally by the [`report!`](crate::report!) macro when you use the
...
/// # Implementation
///
/// You don't need to implement this trait manually - it's automatically available for
...
///
/// This trait defines the ownership behavior for [`Report<C, O, T>`](crate::Report) instances.
/// It's implemented for [`Mutable`] and [`Cloneable`], which control whether reports can be
/// mutated or cloned.
///
/// # Ownership Modes
///
/// - [`Mutable`]: Indicates unique ownership. The report can be mutated but not cloned.
///   Methods like [`attach`](crate::Report::attach) and [`as_mut`](crate::Report::as_mut)
///   are only available in this mode.
///
/// - [`Cloneable`]: Indicates shared ownership via reference counting. The report can be
///   cloned cheaply but cannot be mutated since there may be multiple references.
///
/// # Associated Types
///
/// The `RefMarker` associated type determines what kind of reference you get when calling
/// [`as_ref`](crate::Report::as_ref):
/// - For [`Mutable`], this is [`Uncloneable`] (the reference cannot use `clone_arc`)
/// - For [`Cloneable`], this is [`Cloneable`] (the reference can use `clone_arc`)
///
/// # Implementation
///
/// This trait is sealed and cannot be implemented outside of this crate. You should use
/// the provided implementations for [`Mutable`] and [`Cloneable`].
pub trait ReportOwnershipMarker: sealed_report_ownership_marker::Sealed {
    ...
    ///
    /// - For [`Mutable`]: Returns [`ReportRef<C, Uncloneable>`](crate::ReportRef) because
    ///   the underlying report has unique ownership
    /// - For [`Cloneable`]: Returns [`ReportRef<C, Cloneable>`](crate::ReportRef) because
    ///   the underlying report already uses shared ownership
...
///
/// This trait defines the ownership behavior for [`ReportRef<C, O>`](crate::ReportRef)
/// instances. It's implemented for [`Cloneable`] and [`Uncloneable`], controlling whether
/// the reference provides the [`clone_arc`](crate::ReportRef::clone_arc) method to obtain
/// an owned report.
///
/// Note that `ReportRef` itself is always `Copy` and `Clone` regardless of this marker.
/// The ownership marker only controls access to the `clone_arc` method.
///
/// # Reference Modes
///
/// - [`Cloneable`]: The reference provides [`clone_arc`](crate::ReportRef::clone_arc) to
///   clone the underlying `Arc` and get an owned [`Report<C, Cloneable, T>`](crate::Report).
///
/// - [`Uncloneable`]: The reference does not provide `clone_arc`. This is used for references
///   to uniquely-owned reports, preserving the uniqueness guarantee.
///
/// # Implementation
///
...
///
/// This trait defines whether a report can cross thread boundaries. It's implemented for
/// [`SendSync`] and [`Local`], which control the thread-safety requirements of reports.
///
/// # Thread Safety Modes
///
/// - [`SendSync`]: Reports (and all their contents) are `Send + Sync`, meaning they can
///   be safely sent to other threads and shared between threads.
///
/// - [`Local`]: Reports contain thread-local data that is not `Send` or `Sync`. These
///   reports cannot cross thread boundaries.
///
/// # Implementation
///
/// This trait is sealed and cannot be implemented outside of this crate.
...
///
/// This trait constrains what types can be used as context or attachment data based on
/// the thread-safety requirements of the report.
///
/// # Implementations
///
/// - For `T = Local`: Implemented for all types that implement [`ObjectMarker`], regardless
///   of their `Send`/`Sync` status. This allows using types like `Rc` in local reports.
///
/// - For `T = SendSync`: Implemented only for types that implement [`ObjectMarker`] and
///   are also `Send + Sync`. This ensures thread-safe reports only contain thread-safe data.
///
/// # Usage
///
/// This trait is used internally by the library to enforce that you can't accidentally
/// create a supposedly thread-safe report containing non-thread-safe data:
///
/// ```compile_fail
/// use std::rc::Rc;
/// use rootcause::prelude::*;
