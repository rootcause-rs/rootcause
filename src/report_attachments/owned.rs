...
impl<T> ReportAttachments<T> {
    ...
    pub fn iter_mut(&mut self) -> ReportAttachmentsIterMut<'_> {
        // SAFETY:
        // 1. The iterator does not allow structural mutation of the collection
        //    (adding/removing elements). `Vec::iter_mut()` only provides mutable access
        //    to existing elements, not the ability to change the vector's length.
        // 2. The iterator yields `ReportAttachmentMut<'_, Dynamic>` which only allows
        //    in-place mutation of attachment data through methods like `inner_mut()`.
        //    It does not expose any way to replace an entire attachment with a
        //    different type, so the `Send + Sync` invariant (when `T = SendSync`) is
        //    preserved.
        let raw = unsafe {
