...
//!         crate_root_paths: &[".cargo/registry/"], // Path roots for crate identification
...
/// Checks if a path component is a versioned crate directory (e.g. `tokio-1.37.0`)
/// and returns the crate name portion if valid.
fn extract_versioned_crate_name(component: &str) -> Option<&str> {
...
/// Extracts a crate name from a file path using configured root paths.
///
/// Scans path components after a matched root for a versioned crate directory.
/// Falls back to the first component if no versioned directory is found.
fn crate_name_from_path(path: &str, roots: &[&str]) -> Option<String> {
    for root in roots {
        let Some(idx) = path.find(root) else {
            continue;
        };
        let after = &path[idx + root.len()..];
        let mut first_component = None;
        for component in after.split(&['/', '\\']) {
            if component.is_empty() {
                continue;
            }
            if let Some(name) = extract_versioned_crate_name(component) {
...
///     // Identify crates by path for filtering
///     crate_root_paths: &[".cargo/registry/"],
...
pub struct BacktraceFilter {
    ...
    /// Path substrings under which crate directories can be found.
    /// When a frame's path contains one of these substrings, the next path
    /// component is used as the crate name (stripping version suffixes like
    /// `-1.37.0`).
    pub crate_root_paths: &'static [&'static str],
    ...
    pub fn capture(filter: &BacktraceFilter) -> Option<Self> {
        ...
        backtrace::trace(|frame| {
            backtrace::resolve_frame(frame, |symbol| {
                ...
                let effective_crate = crate_name_from_path(&path_str, filter.crate_root_paths);

                if initial_filtering {
                    if let Some(cur_crate) = effective_crate.as_deref()
                        && filter
                            .skipped_initial_crates
                            .iter()
                ...
                let frame_path = FramePath::new(filename_raw, effective_crate.as_deref());
        ...
        while let Some(last) = entries.last() {
            match last {
                BacktraceEntry::Frame(frame) => {
                    let mut skip = false;
                    let effective_crate = frame
                        .frame_path
                        .as_ref()
                        .and_then(|fp| crate_name_from_path(&fp.raw_path, filter.crate_root_paths));
                    if let Some(cur_crate) = effective_crate.as_deref()
                        && filter
                            .skipped_final_crates
                            .iter()
...
impl FramePath {
    fn new(path: BytesOrWideString<'_>, crate_name: Option<&str>) -> Self {
